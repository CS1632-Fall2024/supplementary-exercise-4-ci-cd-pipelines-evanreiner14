...
public class RentACatImpl implements RentACat {
	...
	public boolean returnCat(int id) {
		// TODO: Fill in
		Cat catToRet = getCat(id);

		if(catToRet == null){
			return false;
		}
		
		if(catToRet.getRented() == true){
			catToRet.returnCat();
			System.out.println("Welcome back, " + catToRet.getName()+ "!");
			return true;
		}
		System.out.println(catToRet.getName() + " is already here!");
		return false;
	}
	...
	public String listCats() {
		...
		for(Cat c : cats){
			if(c.getRented() == false){
				retStr.append("ID " + c.getId() + ". ");
				retStr.append(c.getName() + "\n");
			}
		}
